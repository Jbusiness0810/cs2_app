...
- Sorts: discount, popularity, cross-market spread, price, and rarity, each reversible with the direction button next to the sort dropdown.
- An item type filter (Souvenir, StatTrak, knives and gloves, normal) also runs server-side over the full list and combines with the rarity filter. Souvenir plus the default discount sort surfaces underlisted souvenirs, priced against Buff163 or Steam references since souvenirs rarely carry marketplace suggested prices.
